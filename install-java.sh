...
mkdir /usr/lib/jvm
cd /usr/lib/jvm
curl -L -b "oraclelicense=a" http://download.oracle.com/otn-pub/java/jdk/8u51-b16/server-jre-8u51-linux-x64.tar.gz -O
tar zxvf server-jre-8u51-linux-x64.tar.gz
chown root:root -R jdk1.8.0_51
ln -s /usr/lib/jvm/jdk1.8.0_51 latest
...
# clean up
rm server-jre-8u51-linux-x64.tar.gz 
